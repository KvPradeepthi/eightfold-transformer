...
def merge(csv_rec, resume_rec):
    ...
    # Map raw experience and education
    experience = [
        {"summary": e, "source": "resume", "confidence": RESUME_CONFIDENCE}
        for e in r.get("experience_raw", [])
    ]
    education = [
        {"raw": e, "source": "resume", "confidence": RESUME_CONFIDENCE}
        for e in r.get("education_raw", [])
    ]

    # Overall confidence is average of field confidence scores
...
def merge_all(csv_records, resume_record):
    ...
    resume_emails = set(e.lower() for e in (resume_record.get("emails") or []) if e)

    # A resume contains data if it has a name, emails, phones, or skills
    has_resume_data = bool(
        resume_record and (
            resume_record.get("full_name") or
            resume_record.get("emails") or
            resume_record.get("phones") or
            resume_record.get("skills")
        )
    )

    for csv_rec in csv_records:
        csv_emails = set(e.lower() for e in (csv_rec.get("emails") or []) if e)
        if has_resume_data and (csv_emails & resume_emails):
            results.append(merge(csv_rec, resume_record))
            resume_matched = True
        else:
            results.append(merge(csv_rec, {}))

    if has_resume_data and not resume_matched:
...
if __name__ == "__main__":
    ...
    csv_recs = parse_csv("input/recruiter.csv")
    res_rec  = parse_resume("input/resume.pdf")
    print(json.dumps(merge_all(csv_recs, res_rec), indent=2))